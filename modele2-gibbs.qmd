...
gibbs2 = function(nchain, n_ops, n_death, N_hospitals, alpha, beta, sigma_mu, prop_sd){
  ...
  colnames(chain) = c(paste("b",1:12), "mu", "tau")
...
chain = gibbs2(nchain, n_ops, n_death, N_hospitals, alpha, beta, sigma_mu, prop_sd)$chain
```
...
```{r}
failure = gibbs2(nchain, n_ops, n_death, N_hospitals, alpha, beta, sigma_mu, prop_sd)$failure_rank
...
```{r}
for (i in 1:12) {
  hist(failure[,i], freq = FALSE, main = paste("p[", i, "]"), xlab = "rang", breaks = 12)
}
```
